...
retrieve(graphKey, "fetch api NBA") :
  ...
  1. searchNodes("fetch api NBA", queryEmbedding?)
     ...
     → Sinon : Tokenise la query, score chaque node sur ses champs
       (_key, type, data, config.displayName, config.description)
  ...
  7. Assemblage : tronque data (2000 chars)
...
| `truncateData` | 2000 | Taille max des data serialisees (chars) |
...
| `RelevantNode` | Node condense : _key, type, typeName, sheet, sheetName, handles, dataSummary |
...
Methodes privees :
- `nodeToRelevant()` — tronque data, resout le nom du sheet et du type
...
WebSocketManager.saveGraphChanges()  ← auto-save toutes les 30s
       ...
       └── [fire-and-forget] generateNodeEmbeddings()
              ...
              │     → true (type, data, handles) → continue
              │
...
| `read_node_detail` | `{ nodeKey }` | Detail complet : type, data, handles, position |
| `read_node_config` | `{ typeKey }` | Definition d'un type : code process (JS), handles, taille par defaut, description |
...
| `read_subgraph` | `{ nodeKeys, fields?, includeConfigs?, includeEdges? }` | Lecture batch de plusieurs nodes avec champs configurables |

**Architecture :**
- `getReadToolDefinitions()` → retourne les 8 outils au format OpenAI function calling
...
| `generateThreadId()` | Async. Genere un ID unique via `createUniqueToken` (collision-free, crypto-secure). Fallback `randomBytes` en mode memoire-seule. |
...
### `actionConverter.ts` — Convertisseur async
...
Les cles de nodes/edges sont generees via `createUniqueToken` (crypto-secure, collision-free).

**Fonction principale :**

```typescript
async convertAction(action: ProposedAction, graphKey: string, defaultSheetId?: string, configs?: NodeTypeConfig[]): Promise<ActionConversionResult>
```
...
| `create_node` | 1 `Node<unknown>` dans `nodesToCreate` (key generee via `createUniqueToken`, collision-free). Utilise `createNodeFromConfig` si configs fourni, sinon defaults hardcodes |
| `create_edge` | 1 `Edge` dans `edgesToCreate` (key generee via `createUniqueToken`) |
| `delete_node` | key dans `nodeKeysToDelete` |
| `delete_edge` | key dans `edgeKeysToDelete` |
| `batch` | Fusion recursive des sous-actions |

**Proprietes :** Async (utilise `createUniqueToken` pour la generation de cles). Le `defaultSheetId` est utilise pour les actions qui n'ont pas de champ `sheet` (move_node, update_node, delete_node, delete_edge).
...
|---------|-------|-------------|
| `utils.test.ts` | 25 | `createNodeEmbeddingText` (type, data, truncation, cas vides), `hasNodeContentChanged` (position-only, size-only, type/data/handles changes), `truncate`, `summarizeHandles` |
...
- Recherche par scoring de tokens (tokenise query, match sur _key/type/data/config)
