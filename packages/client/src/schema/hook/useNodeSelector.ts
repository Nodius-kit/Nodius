...
import {deepEqual} from "@nodius/utils";
import {Edge} from "@nodius/utils";
...
export const useNodeSelector = () => {
    ...
    const mouseDown = (e:MouseEvent) => {
        ...
        const mouseMove = (e:MouseEvent) => {
            if(!selectingState.current.isSelecting || !selectingState.current.rect) return;
            ...
            if (!selectingState.current.rect.parentElement) return;

            // Throttle the heavy work (node/edge selection) to animation frames
