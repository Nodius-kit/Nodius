...
import {HtmlBuilderCategoryType, HtmlBuilderComponent, HtmlList, HtmlObject} from "@nodius/utils";
...
import {ObjectStorage} from "@nodius/process";
...
export const LeftPaneComponentTree = memo(({
    ...
    const flatNodes = useMemo(() => {
        ...
        if (Project.state.editedHtml) {
            const node = Project.state.editedHtml.htmlRenderContext.retrieveNode();
            if(node) {
                traverse(Project.state.editedHtml.htmlRenderContext.retrieveHtmlObject(node), undefined);
            }
        }
...
const TreeNode = memo(({ object, depth, ...props }: TreeNodeProps) => {
    ...
    const onMouseLeave = useCallback((evt: React.MouseEvent) => {
        ...
        Project.state.editedHtml?.htmlRenderContext.htmlRender.pushBuildingInteractEvent('hover', undefined);
    ...
    const onMouseDown = useCallback((evt: React.MouseEvent) => {
        ...
        const onMouseUp = async (evt:MouseEvent) => {
            ...
            if(hoverElement && hoverElement.getAttribute("data-tree-object") != undefined) {
                ...
                let objectTo = searchElementWithIdentifier(identifier, Project.state.editedHtml.htmlRenderContext.retrieveHtmlObject(node), instructionTo);

                const instructionFrom = new InstructionBuilder(); // get path of the current object
                let objectFrom = searchElementWithIdentifier(object.identifier, Project.state.editedHtml.htmlRenderContext.retrieveHtmlObject(node), instructionFrom);
