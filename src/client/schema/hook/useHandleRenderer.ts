...
import {deepCopy, disableTextSelection, enableTextSelection, Point} from "../../../utils/objectUtils";
...
import {ProjectContext, ProjectContextProps} from "../../hooks/contexts/ProjectContext";
import {useEdgeHandler} from "./useEdgeHandler";
import {useStableProjectRef} from "../../hooks/useStableProjectRef";
import {getHandleInfo, getHandlePosition} from "../../../utils/graph/handleUtils";
...
export function useHandleRenderer({getNode}: useHandleRendererOptions) {
    ...
    const mouseMove = useCallback((evt:MouseEvent) => {
        ...
        lastFrameId.current = requestAnimationFrame(() => {
            ...
            if (!projectRef.current.state.editedNodeConfig) return;

            const posX = evt.clientX;
            const posY = evt.clientY;


            const configNodeId = "0";

            const nodeOverlay = document.querySelector('[data-node-schema-element="'+configNodeId+'"]');
            ...
            if (isOutside) {
                ...
                if (!activeSideConfigPanel.current) {
                    const node = getNode(configNodeId);
                    if(!node) return;
    ...
    }, [Project.state.editedNodeConfig, mouseMove]);
