...
export const useSocketSync = () => {
    ...
    const openHtmlClass = useCallback(async (html:HtmlClass, graph?:Graph):Promise<ActionContext> => {
        ...
        setSelectedSheetId(selectedSheetId); // select first sheet id
        Project.dispatch({
            field: "graph",
            value: htmlGraph
        });
    ...
    const handleIntructionToGraph = useCallback(async (WSMessage:WSApplyInstructionToGraph, beforeApply?: BeforeApplyInstruction):Promise<{status:boolean, error?:string}> => {
        if(!Project.state.graph || !selectedSheetId) return {
            status: false,
        ...
        for(const instruction of WSMessage.instructions) {
            if(instruction.nodeId) {
                const node = Project.state.graph.sheets[selectedSheetId].nodeMap.get(instruction.nodeId);
                if(!node) {
                    return {
                        status: false,
                ...
                const newNode = applyInstruction(node, instruction.i, beforeApply);
                if(newNode.success) {
                    Project.state.graph.sheets[selectedSheetId].nodeMap.set(instruction.nodeId, newNode.value);
                    return {
                        status: true,
                    }
                } else {
                ...
                const oldNEdge = findEdgeByKey(Project.state.graph.sheets[selectedSheetId].edgeMap, instruction.edgeId);
                if(!oldNEdge) {
                    return {
                        status: false,
                ...
                const newEdge = applyInstruction(oldNEdge, instruction.i, beforeApply);
                if(newEdge.success) {
                    ...
                    if(edges[0].target) {
                        ...
                        let edgeListTarget = Project.state.graph.sheets[selectedSheetId].edgeMap.get(targetKey) ?? [];
                        edgeListTarget = edgeListTarget.filter((e) => e._key !== edges[0]._key);
                        if(edgeListTarget.length > 0) {
                            Project.state.graph.sheets[selectedSheetId].edgeMap.set(targetKey, edgeListTarget);
                        } else {
                            Project.state.graph.sheets[selectedSheetId].edgeMap.delete(targetKey);
                        }
                    }

                    if(edges[0].source) {
                        const sourceKey = `source-${edges[0].source}`;
                        let edgeListSource = Project.state.graph.sheets[selectedSheetId].edgeMap.get(sourceKey) ?? [];
                        edgeListSource = edgeListSource.filter((e) => e._key !== edges[0]._key);
                        if(edgeListSource.length > 0) {
                            Project.state.graph.sheets[selectedSheetId].edgeMap.set(sourceKey, edgeListSource);
                        } else {
                            Project.state.graph.sheets[selectedSheetId].edgeMap.delete(sourceKey);
                        }
                    }
    ...
    }, [Project.state.graph, Project.state.editedHtml, selectedSheetId]);

    const updateHtml = useCallback(async (instruction:Instruction, options?:UpdateHtmlOption): Promise<ActionContext> => {
        ...
        if(response && response._response) {
            if(response._response.status) {
                const instructionOutput = await handleIntructionToGraph(response,(objectBeingApplied) => {
                ...
                if(instructionOutput.status) {
                    ...
                    const newNode = Project.state.graph!.sheets[selectedSheetId!].nodeMap.get(Project.state.editedHtml.node._key)!;
    ...
    }, [Project.state.editedHtml, sendMessage]);
